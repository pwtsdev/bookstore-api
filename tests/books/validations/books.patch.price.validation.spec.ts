...
import { PRICE_INCORRECT_DATA } from '@const/response.errors.const';
import { MAX_PRICE, MIN_PRICE } from '@const/validation.const';
...
test.describe('PATCH /books 4xx', { tag: ['@books', '@validation', '@4xx'] }, () => {
  ...
  test('empty price', async () => {
    const bookPayload = { price: '' } as unknown as BookPayload;
  ...
  test('below nim price', async () => {
    const bookPayload = { price: MIN_PRICE - 0.5 } as BookPayload;
  ...
  test('above max price', async () => {
    const bookPayload = { price: MAX_PRICE + 1 } as BookPayload;
  ...
  test('invalid format', async () => {
    const bookPayload = { price: 'ten' } as unknown as BookPayload;
