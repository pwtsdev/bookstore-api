...
import { AVAILABLE_INCORRECT_DATA } from '@const/response.errors.const';
import { MAX_AVAILABLE, MIN_AVAILABLE } from '@const/validation.const';
...
test.describe('PATCH /books 4xx', { tag: ['@books', '@validation', '@4xx'] }, () => {
  ...
  test('empty available', async () => {
    const bookPayload = { available: '' } as unknown as BookPayload;
    ...
    expect(responseBody.message).toContain(AVAILABLE_INCORRECT_DATA);
  });

  test('below nim available', async () => {
    const bookPayload = { available: MIN_AVAILABLE - 0.5 } as BookPayload;
  ...
  test('above max available', async () => {
    const bookPayload = { available: MAX_AVAILABLE + 1 } as BookPayload;
  ...
  test('invalid format', async () => {
    const bookPayload = { available: 'ten' } as unknown as BookPayload;
