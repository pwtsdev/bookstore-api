/* eslint-disable playwright/max-expects */
import { BookResponse } from '@api-models/books/book.model';
import { HTTP_202_ACCEPTED, HTTP_204_NO_CONTENT } from '@const/http.status.codes.const';
import { expect, test } from '@fixtures/api.fixture';
import { getHeadersWithToken } from '@helpers/auth.helper';
import { getFormDataWithBookCover } from '@helpers/form.data.helper';
import { parseResponse } from '@helpers/parse.response.helper';
import { statusCode } from '@helpers/response.status.helper';
import { booksCoverUrl } from '@helpers/url.helper';
import { deleteRequest } from '@requests/delete.request';
import { patchRequest } from '@requests/patch.request';
import { deleteAuthorAPIStep } from 'src/api/steps/authors/delete.author.step';
import { createBookAPIStep } from 'src/api/steps/books/create.book.step';
import { deleteBookAPIStep } from 'src/api/steps/books/delete.book.step';
import { readBookByIdAPIStep } from 'src/api/steps/books/read.book.step';
import { createAdminTokenAPIStep } from 'src/api/steps/token/create.admin.token.step';

test.describe('PATCH /books/:id/cover 2xx', { tag: ['@book-cover', '@slow', '@e2e'] }, () => {
  let existingBook: BookResponse;
  let bookId: number;
  let authorId: number;
  let adminToken: string;
  let headers: Record<string, string>;

  test.beforeEach('create test data', async () => {
    existingBook = await createBookAPIStep();
    bookId = existingBook.id;
    authorId = existingBook.authors[0].id;

    adminToken = await createAdminTokenAPIStep();
    headers = getHeadersWithToken(adminToken);
  });

  test.afterEach('delete test data', async () => {
    if (bookId) await deleteBookAPIStep(bookId);
    if (authorId) await deleteAuthorAPIStep(authorId);
  });

  test('update and delete book cover', async () => {
    await test.step('update book cover', async () => {
      const formData = getFormDataWithBookCover();

      expect(existingBook.coverId).toBeNull();

      const response = await patchRequest(booksCoverUrl(bookId), formData, headers);
      expect(statusCode(response)).toBe(HTTP_202_ACCEPTED);

      const responseBody = await parseResponse<BookResponse>(response);
      expect(responseBody.coverId).not.toBeNull();
    });

    await test.step('read updated book after PATCH', async () => {
      const updatedBook = await readBookByIdAPIStep(bookId);
      expect(updatedBook.coverId).not.toBeNull();
    });

    await test.step('delete book cover', async () => {
      const response = await deleteRequest(booksCoverUrl(bookId), headers);
      expect(statusCode(response)).toBe(HTTP_204_NO_CONTENT);
    });

    await test.step('read updated book after DELETE', async () => {
      const updatedBook = await readBookByIdAPIStep(bookId);
      expect(updatedBook.coverId).toBeNull();
    });
  });
});
